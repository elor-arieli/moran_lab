...
class pickle_loader(object):
            ...
            for neural_spike_times in self.data['neurons']:
                ...
                for i in range(len(pieces) - 1):
                    ...
                    if len(spikes)>0:
                        ...
                        CV_over_time.append(0)
                ax.plot(savitzky_golay(CV_over_time,9,3),label='{}-{}'.format(neural_spike_times[0],neural_spike_times[1]))
    ...
    def calc_CV_over_time_for_neuron(self, elec, cluster, average_ever_x_minutes=5,save=False, pre_title=None):
        ...
        fig.clf()
        ax = fig.add_subplot(111)
        ax.plot(savitzky_golay(CV_over_time,9,3))
        if save:
            if pre_title is not None:
            ...
            for neural_spike_times in self.data['neurons']:
                ...
                for i in range(len(pieces) - 1):
                    ...
                    if len(spikes) > 0:
                        ...
                        FF_over_time.append(0)
                ax.plot(savitzky_golay(FF_over_time,9,3), label='{}-{}'.format(neural_spike_times[0], neural_spike_times[1]))
    ...
    def calc_FF_over_time_for_neuron(self, elec, cluster, average_ever_x_minutes=5, save=False, pre_title=None):
        ...
        fig.clf()
        ax = fig.add_subplot(111)
        ax.plot(savitzky_golay(FF_over_time,9,3))
        if save:
            if pre_title is not None:
...
def PCA_2D_for_population_batch_responses(ax1, pca_data_array, neuron_num_list, taste='sacc', title='', normalize=False):
    ...
    for batch_num, batch in enumerate(pca_data_array[taste_to_num[taste]]):
        batch_lengths.append(len(batchcalc))
        if batch_num == 0:
            batch_mat = np.array(batch)
        else:
            #             print(batch_mat.shape)
